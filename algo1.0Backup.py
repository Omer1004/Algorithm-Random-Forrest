...
def prepare_xy(preprocessed_data):
    X = pd.concat([df.loc[:, ['EMA_9', 'EMA_72', 'EMA_89', 'RSI', 'MACD', 'MACD_signal', 'MACD_hist']] for df in
                   preprocessed_data.values()], axis=0)
    y = pd.concat([df.loc[:, 'Target'] for df in preprocessed_data.values()], axis=0)
    return X, y
...
def tune_hyperparameters(X_train, y_train):
    ...
    xgb_grid = GridSearchCV(estimator=xgb, param_grid=param_grid, cv=3, verbose=2, n_jobs=-1)
...
def tune_hyperparameters2(X_train, y_train):#280 min to run by rf grid
...
def run_trading_strategy(model, data_dict, trade_records):
    # Initialize portfolio
    portfolio = {stock: 0 for stock in data_dict.keys()}
    initial_cash = 100000  # starting amount in dollars
    cash = initial_cash
    st = time.time()

    # Create list of unique trading days
    trading_days = sorted(list(set().union(*[data.index.tolist() for data in data_dict.values()])))

    for day in trading_days:
        # At the start of the day, make buying decisions
        for ticker, data in data_dict.items():
            if day in data.index:
                current_day = data.loc[day]

                # Get model's prediction for the day
                X_current = pd.DataFrame(
                    current_day[
                        ['EMA_9', 'EMA_72', 'EMA_89', 'RSI', 'MACD', 'MACD_signal', 'MACD_hist']].values.reshape(1, -1),
                    columns=['EMA_9', 'EMA_72', 'EMA_89', 'RSI', 'MACD', 'MACD_signal', 'MACD_hist'])
                prediction = model.predict(X_current)[0]
                # If the model predicts an up movement, buy the stock
                if prediction == 1 and cash >= current_day['Open']:
                    # Buy as much as possible while not exceeding 5% of the total cash
                    max_cash_to_use = max(0.05 * cash, current_day['Open'])
                    shares_to_buy = min(cash // current_day['Open'], max_cash_to_use // current_day['Open'])
        ...
        # At the end of the day, sell all stocks in the portfolio
        for ticker, data in data_dict.items():
            if portfolio[ticker] > 0 and day in data.index:
                current_price = data.loc[day, 'Close']
                cash_inflow = (portfolio[ticker] * current_price) -5# -5 is for commision
                cash += cash_inflow

                # Record the trade exit and calculate profit/loss
                trade_records.loc[
                    (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'exit_price'] = current_price
                trade_records.loc[(trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'profit_loss'] = \
                    cash_inflow - trade_records.loc[
                        (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'position_size']

                # Calculate profit/loss percentage
                trade_records.loc[
                    (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'profit_loss_pct'] = \
                    (trade_records.loc[
                         (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'exit_price'] - \
                     trade_records.loc[
                         (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'entry_price']) / \
                    trade_records.loc[
                        (trade_records['date'] == day) & (trade_records['ticker'] == ticker), 'entry_price'] * 100

                # Reset the ticker's position in the portfolio
                portfolio[ticker] = 0

    # Print the remaining cash and trade records
    print("Cash remaining:", cash)
    print("Profit/Loss:", cash - initial_cash)
    print("Trade records:", trade_records)

    trade_records_dict = trade_records.copy()
    trade_records_dict['date'] = trade_records_dict['date'].dt.strftime('%Y-%m-%d')
    trade_records_dict = trade_records_dict.to_dict("records")
    #
    ...
    with open('trading_records.json', 'w') as f:
        json.dump(trade_records_dict, f)
    ...
    return cash, trade_records
    et = time.time()
    print("\nTime it took to run in min:", (et - st) / 60)
...
def main():
    ...
    if run_stratagy_load:
        final_portfolio_value, trade_records_dict = run_trading_strategy(rf_best, data_with_indicators,trade_records)
        print("Value of portfolio is ",final_portfolio_value)
...
def show_trade_records_screen(trade_records, total_cash, profit_loss, avg_profit_usd, avg_profit_pct, max_loss, max_profit):
    ...
    cash_label = Label(records_window, text=f"Cash: {total_cash}")
    cash_label.pack()

    profit_loss_label = Label(records_window, text=f"Total Profit/Loss: {profit_loss}")
    profit_loss_label.pack()
    avg_profit_usd_label = Label(records_window, text=f"Average Profit per Trade (USD): {avg_profit_usd}")
    avg_profit_usd_label.pack()

    avg_profit_pct_label = Label(records_window, text=f"Average Profit per Trade (%): {avg_profit_pct}")
    avg_profit_pct_label.pack()

    max_loss_label = Label(records_window, text=f"Max Loss on a Single Trade: {max_loss}")
    max_loss_label.pack()

    max_profit_label = Label(records_window, text=f"Max Profit on a Single Trade: {max_profit}")
...
def main_gui(model, stock_list, X_test, y_test, rf_best, normal_model_predictions, best_model_predictions, trade_records,totalcashAPNL):
    ...
    Button(root, text="Show Trade Records",
           command=lambda: show_trade_records_screen(
               trade_records,
               totalcashAPNL['cash'],
               totalcashAPNL['profit_loss'],
               totalcashAPNL['avg_profit_usd'],
               totalcashAPNL['avg_profit_pct'],
               totalcashAPNL['max_loss'],
               totalcashAPNL['max_profit']
           )).pack()
...
def getPredictionsOfBestModelForApi():
    stock_list = ['QQQ', 'AAPL', 'TSLA', 'GOOGL', 'NVDA', 'META', 'MSFT', 'AMZN', 'AMD','NFLX','BABA']
    period = '5y'
    interval = '1d'
