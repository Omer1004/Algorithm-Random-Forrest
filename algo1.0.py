...
def expanded_grid_search(X_train, y_train):#takes too much time
    #Fitting 3 folds for each of 6300 candidates, totalling 18900 fits 15 second avarage
    param_grid = {
...
def run_trading_strategy(model, data_dict, trade_records,X_test):
    ...
    trading_days = sorted(list(set(X_test.index.tolist())))

    for day in trading_days:
        # At the start of the day, make buying decisions
        for ticker in data_dict.items():
...
def run_trading_strategy2(model, data_dict, trade_records,X_test):
    # Initialize portfolio
    portfolio = {stock: 0 for stock in data_dict.keys()}
    initial_cash = 100000  # starting amount in dollars
    cash = initial_cash
    ...
    for day in trading_days:
        # At the start of the day, make buying decisions
        for ticker, data in data_dict.items():
            if day in data.index and day in X_test.index:
                current_day = data.loc[day]
...
def main():
    ...
    for stock in stock_list:
        ...
        X_train_stock, X_test_stock, y_train_stock, y_test_stock = train_test_split(X_stock, y_stock, test_size=0.1,
    ...
    if best_params_load:
        ...
        #best_params = expanded_grid_search(X_train, y_train)
